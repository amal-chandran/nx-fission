...
export async function* nodeExecutor(
  ...
  for await (const event of startBuild(options, context)) {
    ...
    await handleBuildEvent(event, options, context, mappings);
    yield event;
  }
}
...
async function runProcess(
  ...
  const subProcess = fork(
    joinPathFragments(__dirname, 'serve-node'),
    options.args,
    {
      execArgv,
      stdio: 'inherit',
      env: {
        ...process.env,
        NX_FISSION_CONFIG: joinPathFragments(
          context.root,
          options.fissionConfig
        ),
        NX_BUILD_DIR: joinPathFragments(context.root, options.outputPath),
        NX_PORT: '8081',
        NX_HOST: '0.0.0.0',
...
async function handleBuildEvent(
  ...
  if (event.success) {
    await runProcess(event, options, context, mappings);
  }
}
